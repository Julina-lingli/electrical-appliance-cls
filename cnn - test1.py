...
from prepareData1 import loadData
import numpy as np
...
class CNN(object):
    ...
    def defineCNN(self):
        ...
        img = tf.reshape(self.input, [-1, 128, 118, 1])
        # 定义卷积层1和池化层1，其中卷积层1里有20个feature map
        # convPool1的形状为[-1, 12, 12, 20]
        convPool1 = self.defineConvPool(img, filterShape=[3, 3, 1, 20],
            poolSize=[1, 2, 2, 1])
        # 定义卷积层2和池化层2，其中卷积层2里有40个feature map
        # convPool2的形状为[-1, 4, 4, 40]
        convPool2 = self.defineConvPool(convPool1, filterShape=[3, 3, 20, 40],
            poolSize=[1, 2, 2, 1])
        #convPool3 = self.defineConvPool(convPool2, filterShape=[3, 3, 40, 80],poolSize=[1, 2, 2, 1])
        #convPool4 = self.defineConvPool(convPool3, filterShape=[3, 3, 80, 160],poolSize=[1, 2, 2, 1])
        # 将池化层2的输出变成行向量，后者将作为全连接层的输入
        print("1111111111111111111")
        print(convPool1.shape)
        print(convPool2.shape)
        print("1111111111111111111")
        convPool2 = tf.reshape(convPool2, [-1, 40 * 30 * 28])
        # 定义全连接层
        self.out = self.defineFullConnected(convPool2, size=[84, 11])
    ...
    def SGD(self, X, Y, startLearningRate, miniBatchFraction, epoch, keepProb):
        ...
        method = tf.train.GradientDescentOptimizer(0.09)
        ...
        while (step < epoch):
            ...
            '''
            sess.run([optimizer], feed_dict={self.input: X,
                                             self.label: Y, self.keepProb: keepProb})
            step += 1
            # 评估模型效果，并将日志写入文件
    ...
    def fit(self, startLearningRate=0.1, miniBatchFraction=0.01, epoch=20, keepProb=0.5):
        """
        训练模型
        """
        X = self.trainSet["X"]
