...
class CNN(object):
    ...
    def defineCNN(self):
        ...
        img = tf.reshape(self.input, [-1, 128, 118, 2])
        print(img.shape)
        # 定义卷积层1和池化层1，其中卷积层1里有32个feature map
        # convPool1的形状为[-1, 62, 57, 32], [-1, 31, 28, 32],
        convPool1 = self.defineConvPool(img, filterShape=[3, 3, 2, 32], filterStrides = 2,
            poolSize=[1, 2, 2, 1])
        # 定义卷积层2和池化层2，其中卷积层2里有64个feature map
        # convPool2的形状为[-1, 27, 14, 64] [-1,13,12,64]
        convPool2 = self.defineConvPool(convPool1, filterShape=[3, 3, 32, 64],filterStrides = 1,
            poolSize=[1, 2, 2, 1])
        convPool3 = self.defineConvPool(convPool2, filterShape=[3, 3, 64, 128],filterStrides = 1,
            poolSize=[1, 2, 2, 1])
        print(convPool1.shape)
        print(convPool2.shape)
        print(convPool3.shape)
        # 将池化层2的输出变成行向量，后者将作为全连接层的输入
        convPool3 = tf.reshape(convPool3, [-1, 128 * 6 * 5])
        # 定义全连接层
        self.out = self.defineFullConnected(convPool3, size=[126, 11])

    def defineConvPool(self, inputLayer, filterShape, filterStrides, poolSize):
        """
        定义卷积层和池化层
        """
        ...
        # 定义卷积层
        _conv2d = tf.nn.conv2d(inputLayer, weights, strides=[1, filterStrides, filterStrides, 1], padding="VALID")
    ...
    def evaluation(self, epoch):
        ...
        validationAccurary = self._doEval(self.validationSet["X"], self.validationSet["Y"])
        print("验证集的准确率 %.3f" % validationAccurary)
        if validationAccurary > self.accurary :
            prob = self.predict_proba(self.testSet["X"])
            self.testSet["Y"] = np.argmax(prob, 1)
            self.accurary = validationAccurary
...
if do_train:
    ...
    trainImg = data[0]
    sample_number = 0
    print(type(trainImg[sample_number, :-1]))
    ...
    print("trainSet[X].shape:",trainSet["X"].shape)
    validationSet = {"X": validationData, "Y": validationLabel}
    testLabel = np.zeros(data[2].shape[0])
    testSet = {"X":data[2], "Y":testLabel}
    ...
    if os.name == "nt":
        ...
        ann = CNN("logs/mnist_cnn", trainSet, validationSet, testSet)
    ann.fit()
    submission = pd.read_csv(os.path.join("Training_data", "submission_format.csv"))
    print(submission.head(3))
    submission["appliance"] = ann.testSet["Y"]
    print("accu: %.3f" % ann.accurary)
    submission.to_csv(os.path.join("Training_data", "submission.csv"))
